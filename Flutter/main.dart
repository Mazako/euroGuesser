...
class _MainMenuState extends State<MainMenu> {
  ...
  void _validateInput() {
    ...
    if (numCountries == null || numCountries < 1 || numCountries > _countryList.length) {
      setState(() {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 20),
            Text(
              "Number of countries (max ${_countryList.length}):",
              style: TextStyle(fontSize: 18),
            ),
            SizedBox(height: 10),
            TextField(
...
class QuizScreen extends StatefulWidget {
  ...
  QuizScreen({required this.selectedCountries, required this.dataEntryMode});
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  List<String> _generateOptions(String correctAnswer) {
    ...
    while (options.length < 4) {
      String option = widget.selectedCountries[random.nextInt(widget.selectedCountries.length)]["name"]!;
      options.add(option);
    }
